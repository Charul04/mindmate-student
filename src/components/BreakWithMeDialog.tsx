...
const FUN_FACTS = [
  "Did you know? Honey never spoils. Archaeologists found 3,000-year-old honey and it was still edible!",
  "Bananas are berries, but strawberries aren’t.",
  "Your brain is sometimes more active when you’re asleep than when you’re awake.",
  "Ostriches can run faster than horses!",
];
const MINDFULNESS = [
  "Close your eyes, take three deep breaths, and slowly scan your body from head to toe. Notice how you feel. You’re doing great! 💙",
  "Pause and think of 3 things you're grateful for right now. Smile at each one.",
  "Sit back, relax your shoulders, and roll your neck gently. Well done!",
  "Stretch your arms up, then out wide. Hold, then release — you got this!",
];
const CHALLENGES = [
  "Brain teaser: What has keys but can’t open locks? (Answer: A piano.)",
  "Try counting backward from 50 to 1 in sevens! Tricky but fun!",
  "Quick challenge: Close your eyes and try to touch your nose with your index finger!",
  "Teaser: Name three things you can do with water besides drinking it.",
];
...
export default function BreakWithMeDialog({ triggerClassName }: { triggerClassName?: string }) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogTrigger asChild>
        <button className={triggerClassName}>
          <span className="flex items-center mb-2"><Smile className="text-pink-500" size={28} /></span>
          <span className="font-semibold text-indigo-900 text-[1.08rem]">Break with Me</span>
          <span className="text-indigo-900/70 text-sm mt-1">Fun facts, jokes, and mindful breaks.</span>
        </button>
      </DialogTrigger>
          ...
          {step === 0 ? (
            <div className="w-full">
              ...
              <div className="flex flex-col items-stretch gap-2">
                {OPTIONS.map(opt => (
                  <Button key={opt} variant="secondary" className="w-full" onClick={() => handleSelect(opt as BreakType)}>
                    {opt}
                  </Button>
                ))}
              </div>
