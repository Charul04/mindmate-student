...
const images = ["/quote-bg1.jpg", "/quote-bg2.jpg", "/quote-bg3.jpg", "/quote-bg4.jpg"];
...
async function generateAIQuote(topic: string): Promise<{
  quote: string;
  comment: string;
}> {
...
export default function MotivationalQuoteDialog({
  triggerClassName
}: {
  ...
  function handleSaveQuote() {
    setSaved(prev => [...(prev.some(q => q.quote === current.quote && q.comment === current.comment) ? prev : [...prev, current])]);
  }
  ...
  return <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-xs p-3 mx-auto rounded-lg">
        <DialogHeader>
          <DialogTitle>
            <span className="flex items-center gap-2 text-left text-base">
        ...
        <div className="flex flex-col gap-4 items-center w-full">
          ...
          <div className="relative rounded-xl bg-gradient-to-br from-sky-50 via-white to-indigo-50 shadow transition overflow-hidden w-full flex flex-col items-center px-2 py-3 min-h-[78px]">
            ...
            {current.isAI && <span className="inline-block mt-1 text-rose-500 text-xs font-medium">
                AI generated
              </span>}
            {/* Save button */}
            <Button size="sm" variant="ghost" className="absolute right-2 top-2 shadow-md rounded-full text-indigo-500 hover:bg-indigo-100" title="Save this quote" onClick={handleSaveQuote} tabIndex={0}>
              <Save size={16} />
            </Button>
          </div>
        ...
        {/* Saved quotes */}
        {saved.length > 0 && <div className="mt-4 border-t pt-3">
            ...
            <div className="grid gap-2 max-h-32 overflow-y-auto">
              {saved.map((q, i) => <div key={i} className="flex bg-gradient-to-l from-sky-50/70 to-white border border-indigo-100 rounded-lg items-start gap-2 p-2 shadow relative">
                  <div className="flex-1 pr-1">
                    ...
                    {q.isAI && <span className="inline-block mt-1 text-xs text-rose-500">
                        AI
                      </span>}
                  </div>
                  <Button variant="ghost" size="icon" className="w-6 h-6 min-w-0 p-0 text-rose-500 hover:bg-rose-50 absolute right-2 top-2" aria-label="Remove quote" title="Remove this quote" onClick={() => handleRemoveSaved(i)}>
                    <Minus size={16} />
                  </Button>
                </div>)}
            </div>
