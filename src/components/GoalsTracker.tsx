...
export default function GoalsTracker() {
  ...
  const chartData = [{
    period: 'Week',
    total: getTimeBasedGoals('week').length,
    completed: getTimeBasedGoals('week').filter(g => g.status === 'completed').length
  }, {
    period: 'Month',
    total: getTimeBasedGoals('month').length,
    completed: getTimeBasedGoals('month').filter(g => g.status === 'completed').length
  }, {
    period: 'Year',
    total: getTimeBasedGoals('year').length,
    completed: getTimeBasedGoals('year').filter(g => g.status === 'completed').length
  }];
  const statusData = [{
    name: 'Active',
  ...
  return <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Input placeholder="Goal title" value={title} onChange={e => setTitle(e.target.value)} className="mb-2" />
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <Input type="number" placeholder="Target" value={targetValue} onChange={e => setTargetValue(e.target.value)} className="flex-1" />
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {goals.length === 0 ? <p className="text-center text-gray-500 py-8">No goals yet. Create your first goal above!</p> : goals.map(goal => <div key={goal.id} className="border rounded-lg p-4 space-y-2">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {goal.deadline && <Badge variant="outline" className="text-xs">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => handleToggleStatus(goal.id!, goal.status)}>
                  ...
                  {goal.target_value && <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => handleUpdateProgress(goal.id!, Math.max(0, goal.current_value - 1))} disabled={goal.current_value <= 0}>
                          -
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => handleUpdateProgress(goal.id!, Math.min(goal.target_value || goal.current_value + 1, goal.current_value + 1))}>
                          +
                        </Button>
                      </div>
                    </div>}
