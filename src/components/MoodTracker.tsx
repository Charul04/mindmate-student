      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {moodOptions.map(mood => <button key={mood.value} type="button" onClick={() => setValue("mood", mood.value)} className={`flex flex-col items-center p-3 rounded-xl border-2 transition-all hover:scale-105 ${selectedMood === mood.value ? "border-primary bg-primary/10" : "border-gray-200 hover:border-gray-300"}`}>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="description" placeholder="Describe your day or what triggered this feeling..." {...register("description")} className="min-h-[80px]" />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {triggerOptions.map(trigger => <button key={trigger.value} type="button" onClick={() => setValue("trigger", trigger.value)} className={`px-3 py-2 rounded-lg border transition-all ${selectedTrigger === trigger.value ? "border-primary bg-primary/10" : "border-gray-200 hover:border-gray-300"}`}>
                        {trigger.label}
                      </button>)}
                  </div>
                </div>
        ...
        <TabsContent value="journal" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {loading ? <div className="text-center py-8">Loading your entries...</div> : entries.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                ...
                return <div key={entry.id} className="border rounded-lg p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => entry.id && deleteMoodEntry(entry.id)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                        
                        {entry.description && <p className="text-sm bg-gray-50 rounded p-3">{entry.description}</p>}
                        
                        <div className="flex gap-2 items-center">
                          {trigger && <Badge variant="secondary" className={trigger.color}>
                              {trigger.label}
                            </Badge>}
                          {entry.energy_level && <Badge variant="outline">
        ...
        <TabsContent value="calendar" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <Calendar mode="single" selected={selectedDate} onSelect={date => date && setSelectedDate(date)} className="rounded-md border" components={{
                DayContent: ({
                  date
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                return <div key={mood.value} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className={`h-2 rounded-full ${mood.color}`} style={{
                        width: `${percentage}%`
                      }} />
                        </div>
                      </div>
          ...
          {getEntriesForDate(selectedDate).length > 0 && <Card>
              <CardHeader>
                <CardTitle>
              ...
              <CardContent>
                ...
                const mood = moodOptions.find(m => m.value === entry.mood);
                const trigger = triggerOptions.find(t => t.value === entry.trigger);
                return <div key={entry.id} className="border rounded-lg p-3">
